# @marcuscastelo/eslint-config

## 0.4.0

### Minor Changes

- 858ccd7: Release updates for eslint-config, eslint-plugin, tsconfig.

### Patch Changes

- Updated dependencies [858ccd7]
  - @marcuscastelo/eslint-plugin@0.3.0

## 0.3.1

### Patch Changes

- ed02291: Release updates for eslint-config.

## 0.3.0

### Minor Changes

- 811ebb4: Release updates for eslint-config.

## 0.2.3

### Patch Changes

- e8fa6ab: Release updates for eslint-config.

## 0.2.2

### Patch Changes

- 190a4b8: Release updates for eslint-config, tsconfig.

## 0.2.1

### Patch Changes

- e2cda39: Testing workflow
- 1418045: Initial changeset
- Updated dependencies [e2cda39]
- Updated dependencies [1418045]
  - @marcuscastelo/eslint-plugin@0.2.1

## 0.2.0

### Minor Changes

- Initial public PoC release for tsconfig, eslint config, and eslint plugin packages.

### Patch Changes

- Initial changeset
- Updated dependencies
- Updated dependencies
  - @marcuscastelo/eslint-plugin@0.2.0
